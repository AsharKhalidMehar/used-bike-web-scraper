...
let defaultUrl =
  "https://www.pakwheels.com/used-bikes/honda-cb-125f-karachi/419752";
...
const createJSONPayload = ($2, index) => {
  ...
  const year = $2(
    "table[class*='table table-bordered text-center table-engine-detail fs16'] > tbody > tr td:nth-child(1) p"
  ).text();
  const mileage = extractNumberFromText(
    $2("table[class*='table table-bordered text-center table-engine-detail fs16'] > tbody > tr td:nth-child(2) p").text()
  );
  const stroke = $2(
    "table[class*='table table-bordered text-center table-engine-detail fs16'] > tbody > tr td:nth-child(3) p"
  ).text();
  const bodyType = $2("table[class*='table table-featured nomargin'] tbody tr:nth-child(2) td:nth-child(2) a").text();
...
const extractListData = async (url) => {
  try {
    ...
    const divWithUl = $("div[class*='search-listing']");
    const promises = [];
    divWithUl.find("ul").each(function (_, ulElem) {
      $(ulElem)
        .find('li[class*="classified-listing"]')
        .each(async function (i, liElem) {
          const scriptTagText = $(liElem).find("div div:nth-child(2) div div div a").attr('href')
          if (scriptTagText) {
            // const detailsObj = JSON.parse(scriptTagText);
            const detailsUrl = `https://www.pakwheels.com${scriptTagText}`;
